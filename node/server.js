...
app.get('/', function(req, res) {
  ...
  var messages = redisClient.lrange('list_pic', 0, 99, function(err, reply) {
    if(!err) {        
      var result = [];
      // Loop through the list, parsing each item into an object
      for(var text in reply) result.push(JSON.parse(reply[text]));
